...
const WORKOUTS: Record<string, Day[]> = {
  FAT_LOSS: [
    ...
    { day: 'Tue', label: 'Tuesday', focus: 'Cardio + Core', exercises: [
      ...
      { name: 'Russian Twists', sets: '3', reps: '20 reps', rest: '30s', tip: 'Lean back 45 degrees. Keep feet off ground.', yt: 'russian+twists+exercise' },
    ]},
    ...
    { day: 'Thu', label: 'Thursday', focus: 'Full Body Strength', exercises: [
      ...
      { name: 'Reverse Lunges', sets: '3', reps: '10 each leg', rest: '45s', tip: 'Step back far enough to make 90 degree angles.', yt: 'reverse+lunge+form' },
      { name: 'Shoulder Press', sets: '3', reps: '12 reps', rest: '45s', tip: 'Drive straight up. Avoid arching lower back.', yt: 'dumbbell+shoulder+press+form' },
    ]},
  ...
  MUSCLE_GAIN: [
    { day: 'Mon', label: 'Monday', focus: 'Push - Chest / Shoulders / Triceps', exercises: [
      ...
      { name: 'Incline Dumbbell Press', sets: '3', reps: '8-10 reps', rest: '90s', tip: '30-45 degree incline. Full range of motion.', yt: 'incline+dumbbell+press+form' },
    ...
    { day: 'Tue', label: 'Tuesday', focus: 'Pull - Back / Biceps', exercises: [
      ...
      { name: 'Barbell Rows', sets: '4', reps: '8-10 reps', rest: '90s', tip: 'Hinge 45 degrees. Pull bar to belly button.', yt: 'barbell+row+proper+form' },
      ...
      { name: 'Barbell Curls', sets: '3', reps: '10-12 reps', rest: '60s', tip: 'No swinging. Pause at top for a second.', yt: 'barbell+curl+proper+form' },
    ...
    { day: 'Wed', label: 'Wednesday', focus: 'Legs - Quads / Hamstrings / Calves', exercises: [
      ...
      { name: 'Leg Press', sets: '3', reps: '10-12 reps', rest: '90s', tip: 'Full range. Keep knees soft at the top.', yt: 'leg+press+proper+form' },
...
WORKOUTS.RECOMPOSITION = WORKOUTS.FAT_LOSS.map((d, i) =>
  i % 2 === 0 ? d : {
    ...d,
    focus: d.isRest ? d.focus : 'Strength + Cardio Mix',
    exercises: [
      ...(WORKOUTS.MUSCLE_GAIN[i]?.exercises.slice(0, 3) || d.exercises),
      ...d.exercises.slice(0, 2),
    ],
  }
)
...
export default function ExercisePage() {
  ...
  const doneToday = day.exercises.filter((_, i) => doneExercises.has(activeDay + '-' + i)).length
    ...
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '20px' }}>
        <div style={{ background: '#111', border: '1px solid #1e1e1e', borderRadius: '12px', padding: '14px' }}>
          ...
          <div style={{ fontSize: '10px', color: '#444', marginTop: '4px' }}>{Math.ceil(waterOz / 8)} glasses</div>
        </div>
        <div style={{ background: '#111', border: '1px solid #1e1e1e', borderRadius: '12px', padding: '14px' }}>
        ...
        {plan.map((d, i) => {
          ...
          return (
            <button key={i} onClick={() => setActiveDay(i)} style={{
              flex: '0 0 auto', padding: '8px 12px', borderRadius: '10px', fontSize: '12px', cursor: 'pointer',
      ...
      <div style={{ background: '#111', border: '1px solid #1e1e1e', borderRadius: '14px', padding: '16px', marginBottom: '16px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
          {day.exercises.map((ex, i) => {
            ...
            return (
              <div key={i} style={{
                background: done ? 'rgba(34,197,94,0.06)' : '#0d0d0d',
                borderRadius: '10px',
                border: `1px solid ${done ? '#22c55e44' : '#1e1e1e'}`,
                overflow: 'hidden',
                transition: 'all 0.2s',
              }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 14px', cursor: 'pointer' }}
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ fontSize: '11px', color: '#555', marginTop: '2px' }}>
                      {ex.sets !== '-' ? `${ex.sets} sets x ${ex.reps} · rest ${ex.rest}` : ex.reps}
                    </div>
                  </div>
                ...
                {expanded && (
                  <div style={{ padding: '0 14px 14px', borderTop: '1px solid #1a1a1a' }}>
                    ...
                    <a href={`https://www.youtube.com/results?search_query=${encodeURIComponent(ex.yt + ' tutorial')}`}
                      ...
                      style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', fontSize: '12px', fontWeight: 600, color: '#e8e0d0', background: '#1e0000', border: '1px solid #ff444444', borderRadius: '8px', padding: '8px 14px', textDecoration: 'none' }}>
